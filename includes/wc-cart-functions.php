...
function wc_add_to_cart_message( $product_id ) {
	...
	$cart = '<div class="cart_item_inner"><span onclick="closeCartPopup();" class="close_cart_popup">x</span>'.do_shortcode('[woocommerce_cart]').'</div><script>function closeCartPopup(){jQuery(".cart_item_inner").remove();}</script>';
	$message = $cart;
	echo $cart;
	return '';
